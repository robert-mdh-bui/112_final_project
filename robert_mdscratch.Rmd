...
choice18 %>% 
  ...
  summarise(
    ...
    Carrier = sum(CARRIER_DELAY, na.rm = T),
    Weather = sum(WEATHER_DELAY, na.rm = T),
    NAS = sum(NAS_DELAY, na.rm = T),
    Security = sum(SECURITY_DELAY, na.rm = T),
    `Late Aircraft` = sum(LATE_AIRCRAFT_DELAY, na.rm = T)
  ) %>% 
  pivot_longer(
    cols = -c(wkday,type),
    names_to = "Cause",
  ...
  ggplot()+
  geom_col(aes(x=wkday,y=Minutes,fill=Cause))+
  facet_wrap(~type)+
  theme_minimal()+
  scale_fill_viridis_d()+
  labs(
    x = "Day of the Week",
    y = "Total Delayed Minutes"
  )
...
choice18 %>% 
  ...
  ggplot()+
  geom_col(aes(x=OP_UNIQUE_CARRIER,y=Minutes,fill=Cause),position = "dodge")+
  facet_wrap(~type)+
  theme_minimal()+
  scale_fill_viridis_d()+
  labs(
    x = "Airline",
    y = "Average Delayed Minutes"
  )+
  coord_flip()
